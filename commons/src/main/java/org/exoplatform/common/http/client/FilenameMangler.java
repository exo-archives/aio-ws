 ...
 * HTTPClient.NVPair[], HTTPClient.FilenameMangler) Codecs.mpFormDataEncode} may
 * be handed an instance of a class which implements this interface in order to
 * control names of the decoded files or the names sent in the encoded data.
 * 
...
public interface FilenameMangler {
  /**
   * This is invoked by {@link Codecs#mpFormDataDecode(byte[], java.lang.String,
   * java.lang.String, HTTPClient.FilenameMangler) Codecs.mpFormDataDecode} for
   * each file found in the data, just before the file is created and written.
   * If null is returned then the file is not created or written. This allows
   * you to control which files are written and the names of the resulting
   * files.
   * <P>
   * For {@link Codecs#mpFormDataEncode(HTTPClient.NVPair[],
