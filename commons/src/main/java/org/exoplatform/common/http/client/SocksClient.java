...
class SocksClient {
   ...
   */
  private static final Socket connect(String host, int port, InetAddress localAddr, int localPort)
      throws IOException {
  ...
  private void v4ProtExchg(InputStream inp, OutputStream out, String host, int port)
    ...
    case 93: // request rejected: identd reports diff uid
      throw new SocksException("Connection request rejected: "
          + "identd reports different user-id " + "from " + new String(user, 0, user.length - 1));
   ...
   * routines.
   */
  private void negotiate_gssapi(InputStream inp, OutputStream out) throws SocksException,
      IOException {
   ...
   */
  private void negotiate_userpwd(InputStream inp, OutputStream out) throws SocksException,
      IOException {
    byte[] buffer;
    ...
    try {
      auth_info = AuthorizationInfo.getAuthorization(socks_host, socks_port, "SOCKS5", "USER/PASS",
          null, null, true);
    ...
    if (version != 1)
      throw new SocksException("Wrong version received in username/"
          + "password subnegotiation response: " + version + "; expected: 1");
