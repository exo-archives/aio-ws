...
public class URI {
  ...
  public static String canonicalizePath(String path) {
    ...
    if (!((idx = path.indexOf("/.")) != -1 && (idx == len - 2 || path.charAt(idx + 2) == '/' || (path
        .charAt(idx + 2) == '.' && (idx == len - 3 || path.charAt(idx + 3) == '/')))))
      return path;
  ...
  private final void assemblePath(StringBuffer buf, boolean printEmpty, boolean incFragment,
      boolean unescape) {
  ...
  private static final int copyBuf(char[] utf, int utf_idx, int ch, char[] res, int didx,
      BitSet reserved, boolean escapeAll) {
    if (ch >= 0)
      utf[utf_idx++] = (char) ch;
