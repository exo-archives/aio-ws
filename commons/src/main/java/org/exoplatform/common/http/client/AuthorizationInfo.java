...
public class AuthorizationInfo implements Cloneable {
   ...
   */
  public AuthorizationInfo(String host, int port, String scheme, String realm, NVPair params[],
      Object info) {
   ...
   */
  public static AuthorizationInfo getAuthorization(String host, int port, String scheme,
      String realm) {
   ...
   * @return a pointer to the authorization data or null if not found
   */
  public static synchronized AuthorizationInfo getAuthorization(String host, int port,
      String scheme, String realm, Object context) {
    Hashtable AuthList = Util.getList(CntxtList, context);

    AuthorizationInfo auth_info = new AuthorizationInfo(host, port, scheme, realm, (NVPair[]) null,
        null);
   ...
   */
  static AuthorizationInfo queryAuthHandler(AuthorizationInfo auth_info, RoRequest req,
   ...
   */
  static synchronized AuthorizationInfo getAuthorization(AuthorizationInfo auth_info,
      RoRequest req, RoResponse resp, boolean query_auth_h) throws AuthSchemeNotImplException,
      IOException {
    Hashtable AuthList;
    if (req != null)
   ...
   * @exception AuthSchemeNotImplException If thrown by the AuthHandler.
   */
  static AuthorizationInfo getAuthorization(String host, int port, String scheme, String realm,
      RoRequest req, RoResponse resp, boolean query_auth_h) throws AuthSchemeNotImplException,
      IOException {
    return getAuthorization(
        new AuthorizationInfo(host, port, scheme, realm, (NVPair[]) null, null), req, resp,
        query_auth_h);
  }
   ...
   */
  public static void addAuthorization(String host, int port, String scheme, String realm,
      String cookie, NVPair params[], Object info) {
    addAuthorization(host, port, scheme, realm, cookie, params, info, HTTPConnection
        .getDefaultContext());
  }
   ...
   */
  public static void addAuthorization(String host, int port, String scheme, String realm,
      String cookie, NVPair params[], Object info, Object context) {
   ...
   */
  public static void addBasicAuthorization(String host, int port, String realm, String user,
      String passwd) {
    addAuthorization(host, port, "Basic", realm, Codecs.base64Encode(user + ":" + passwd),
        (NVPair[]) null, null);
  }
   ...
   * @param passwd the password
   */
  public static void addDigestAuthorization(String host, int port, String realm, String user,
      String passwd) {
   ...
   *          the same format as value part of a WWW-authenticate response
   *          header field, and may contain multiple authentication challenges.
  ...
  static AuthorizationInfo[] parseAuthString(String challenge, RoRequest req, RoResponse resp)
    ...
    while (true) // get all challenges
    {
      ...
      if (sts == 401)
        ...
        curr = new AuthorizationInfo(req.getConnection().getProxyHost(), req.getConnection()
            .getProxyPort());
  ...
  private static final Vector parseParams(String challenge, char[] buf, int[] pos_ref, int len,
      AuthorizationInfo curr) throws ProtocolException {
    int beg = pos_ref[0];
    int end = pos_ref[1];
    ...
    while (true) {
      ...
      if (beg < len && buf[beg] == '=') // we have a value
      {
        ...
        } else // it's a quoted-string
        {
          ...
          if (end == -1)
            throw new ProtocolException("Bad Authentication header " + "format: " + challenge
                + "\nClosing <\"> for " + "quoted-string starting at position " + beg
                + " not found");
