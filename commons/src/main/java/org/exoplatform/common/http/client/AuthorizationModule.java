...
class AuthorizationModule implements HTTPClientModule {
  ...
  public int requestHandler(Request req, Response[] resp) throws IOException,
    ...
    Proxy: if (con.getProxyHost() != null && !prxy_from_4xx) {
      ...
      guess = (AuthorizationInfo) proxy_auth_list
          .get(con.getProxyHost() + ":" + con.getProxyPort());
      if (guess == null)
        break Proxy;
  ...
  public void responsePhase1Handler(Response resp, RoRequest req) throws IOException {
    ...
    if (resp.getStatusCode() != 401 && resp.getStatusCode() != 407) {
      if (auth_sent != null && auth_from_4xx) {
        try {
          AuthorizationInfo.getAuthorization(auth_sent, req, resp, false).addPath(
              req.getRequestURI());
   ...
   *
   */
  private void handle_auth_challenge(Request req, Response resp) throws AuthSchemeNotImplException,
      IOException {
    // handle WWW-Authenticate

    int[] idx_arr = { auth_lst_idx, // hack to pass by ref
        auth_scm_idx };
    auth_sent = setAuthHeaders(resp.getHeader("WWW-Authenticate"), req, resp, "Authorization",
        idx_arr, auth_sent);
    if (auth_sent != null) {
      auth_from_4xx = true;
    ...
    prxy_sent = setAuthHeaders(resp.getHeader("Proxy-Authenticate"), req, resp,
        "Proxy-Authorization", idx_arr, prxy_sent);
    if (prxy_sent != null) {
      prxy_from_4xx = true;
   ...
   */
  private AuthorizationInfo setAuthHeaders(String auth_str, Request req, RoResponse resp,
      String header, int[] idx_arr, AuthorizationInfo prev) throws ProtocolException,
