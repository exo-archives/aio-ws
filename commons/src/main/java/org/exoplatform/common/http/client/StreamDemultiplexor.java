...
class StreamDemultiplexor implements GlobalConstants {
  ...
  RespInputStream getStream(Response resp) {
    ...
    synchronized (RespHandlerList) {
      for (resph = (ResponseHandler) RespHandlerList.enumerate(); resph != null; resph = (ResponseHandler) RespHandlerList
          .next()) {
        if (resph.resp == resp)
          break;
      }
  ...
  int read(byte[] b, int off, int len, ResponseHandler resph, int timeout) throws IOException {
    ...
    synchronized (this) {
      ...
      try {
        ...
        default:
          throw new Error("Internal Error in StreamDemultiplexor: " + "Invalid cd_type "
              + resph.resp.cd_type);
        }
  ...
  void abort() {
    ...
    synchronized (RespHandlerList) {
       ...
       * boolean). We do *not* call close() because that is synchronized, but we
