...
public class RESTOrganizationServiceJSONImpl extends RESTOrganizationServiceAbstractImpl implements
  ...
  String parentId) {
    ...
    return Response.Builder.created(
        baseURI + "/organization/json/group/info" + id).build();
  }
  ...
  String type) {
    ...
    return Response.Builder.created(
        baseURI + "/organization/json/membership/info/" + id).build();
  }
    ...
    try {
      ...
      if (groupId != null && username != null && type != null) {
        ...
        if (membership != null) {
          ...
          return Response.Builder.withStatus(HTTPStatus.NOT_FOUND).errorMessage(
              "Membership for groupId: '" + groupId + "', username: '" + username
                  + "', membership type: '" + type + "' not found!").build();
        }
        ...
        return Response.Builder.withStatus(HTTPStatus.INTERNAL_ERROR).errorMessage(
            "Username, groupId or membership type should be specified!").build();
      }
  ...
  Integer amount) {
    try {
      ...
      UserListBean user_list = new UserListBean(list.subList(offset, amount_));
      return Response.Builder.ok(user_list, JSON_CONTENT_TYPE).build();
    } catch (Exception e) {
      LOGGER.error("Thrown exception : " + e);
      return Response.Builder.withStatus(HTTPStatus.INTERNAL_ERROR).errorMessage(
  ...
  String parentId) {
    try {
      ...
      if (parentId != null && parentId.length() > 0) {
        ...
        if (parent == null) {
          return Response.Builder.withStatus(HTTPStatus.NOT_FOUND).errorMessage(
              "Parent '" + parentId + "' not found.").build();
        }
