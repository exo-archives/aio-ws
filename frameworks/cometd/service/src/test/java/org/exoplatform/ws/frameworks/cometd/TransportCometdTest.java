 ...
 * @version $Id: $
 */
public class TransportCometdTest extends TestCase {
  /**
   * Class logger.
   */
  ...
  private String cometdConnect(String id) {
    try {
      ...
      String eXoId = id;
      String cometdUrl = TestTools.getBaseURLCometdServer(baseURI + "cometdurl/" + id + "/");
      String u = cometdUrl + "/rest/gettoken/" + id + "/";
      ...
      String dataHandshake = "message={\"channel\":\"/meta/handshake\",\"id\":1,\"exoId\":\""
          + eXoId + "\",\"exoToken\":\"" + userToken + "\"}";
      ...
      String dataConnect = "message={\"channel\":\"/meta/connect\",\"clientId\":\"" + clientId
          + "\",\"connectionType\":\"" + connectionType + "\",\"id\":2}";
      ...
      for (String channel : channels) {
        dataSub = dataSub.concat("{\"channel\":\"/meta/subscribe\",\"subscription\":\"" + channel
      ...
      for (int j = 0; j < repeat; j++) {
        for (int i = 0; i < con; i++) {
          ...
          for (String key : keySet) {
            for (String channel : channels) {
              String u = baseURI + "ext/sendprivatemessage/";
              TestTools.sendMessage("exo" + String.valueOf(i),
                                    channel,
                                    individuals.get(key),
                                    key,
                                    u);
        ...
        for (String key : keySet) {
          String u = baseURI + "ext/sendbroadcastmessage/";
    ...
    public void run() {
      try {
        ...
        while (messages > i + b) {
          ...
          pairs[2] = new NVPair("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
          pairs[3] = new NVPair("Content-Length", Integer.toString(dataConnect.length()));
          HTTPResponse cometdResponse = connection.Post(url.getFile(),
                                                        dataConnect.getBytes(),
                                                        pairs);
